...
function valid_percentage() {
	...
	lower_threshold=${precentage/-*/}
	upper_threshold=${precentage/*-/}
	# Check if setting is a legal range and is value between 1 and 100
	if [[ "$precentage" =~ ^[0-9]+-?[0-9]+$ ]]; then
...
if [[ "$action" == "maintain" ]]; then
	...
	if [ "$is_voltage" = true ]; then
		...
		nohup $battery_binary maintain_synchronous $upper_threshold $subsetting >>$logfile &
	fi
